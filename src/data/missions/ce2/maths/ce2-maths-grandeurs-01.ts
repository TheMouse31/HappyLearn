import { allUniverses, defineMission } from "../../define";

export const MISSION_ID = "ce2-maths-grandeurs-01";

/** CE2 / maths — Grandeurs (périmètre contenances durées). Phase A standard. */
export const mission = defineMission({
  id: MISSION_ID,
  grade: "ce2",
  subject: "maths",
  title: "Grandeurs (périmètre contenances durées)…",
  blurb: "Une mission CE2 : Grandeurs (périmètre contenances durées).",
  available: true,
  version: 1,
  steps: [
    {
      kind: "continue",
      kicker: "Tutoriel",
      progress: 0,
      copy: allUniverses({
        title: "Bienvenue : Grandeurs (périmètre contenances durées)",
        statement: "Dans cette mission de CE2, tu vas travailler : Grandeurs (périmètre contenances durées). Lis bien chaque consigne, utilise l’indice si besoin, puis valide.",
        hint: "Prends ton temps : il n’y a ni note ni classement.",
        caption: "La mission peut commencer.",
      }),
    },
    {
      kind: "continue",
      kicker: "Mise en action",
      progress: 0,
      copy: {
        football: {
          title: "Tu entres en jeu",
          statement: "Les Bleus comptent sur toi. Chaque bonne réponse fait avancer l’action sur le terrain.",
          caption: "Tu prends ta place au milieu du terrain.",
        },
        rugby: {
          title: "Tu entres en jeu",
          statement: "Tes partenaires comptent sur toi. Chaque bonne réponse ouvre un couloir.",
          caption: "Tu prends ta place derrière tes partenaires.",
        },
        equitation: {
          title: "Le chemin du retour",
          statement: "Tu rentres vers l’écurie avec ton cheval. Chaque étape éclaire le sentier.",
          caption: "L’écurie apparaît au bout du sentier.",
        },
        espace: {
          title: "Le retour vers la station",
          statement: "Le module doit rejoindre la station. Chaque réponse corrige la trajectoire.",
          caption: "La station apparaît au loin.",
        },
      },
    },
    {
      kind: "continue",
      kicker: "Objectif",
      progress: 0,
      copy: {
        football: {
          title: "Lis le jeu",
          statement: "Observe, calcule et décide. Chaque étape te rapproche du but. Objectif : Grandeurs (périmètre contenances durées).",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "Observe la défense",
          statement: "Lis les espaces, calcule, puis avance au bon moment. Objectif : Grandeurs (périmètre contenances durées).",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "Choisis le chemin",
          statement: "Observe les bornes, calcule, et garde une allure sûre. Objectif : Grandeurs (périmètre contenances durées).",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "Rétablis la trajectoire",
          statement: "Analyse les signaux, calcule, et prépare l’arrimage. Objectif : Grandeurs (périmètre contenances durées).",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "number",
      kicker: "Étape 1 sur 6 · Durée",
      progress: 1,
      expected: "60",
      distractors: ["30", "100"],
      copy: allUniverses({
        title: "Minutes dans 1 h",
        statement: "Combien de minutes dans 1 heure ?",
        hint: "60.",
        caption: "Tu valides ta réponse.",
      }),
    },
    {
      kind: "number",
      kicker: "Étape 2 sur 6 · Durée",
      progress: 2,
      expected: "90",
      distractors: ["80", "100"],
      copy: {
        football: {
          title: "1 h 30",
          statement: "1 h 30 min = combien de minutes ?",
          hint: "90.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "1 h 30",
          statement: "1 h 30 min = combien de minutes ?",
          hint: "90.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "1 h 30",
          statement: "1 h 30 min = combien de minutes ?",
          hint: "90.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "1 h 30",
          statement: "1 h 30 min = combien de minutes ?",
          hint: "90.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "number",
      kicker: "Étape 3 sur 6 · Angle",
      progress: 3,
      expected: "90",
      distractors: ["45", "180"],
      copy: allUniverses({
        title: "Angle droit",
        statement: "Combien de degrés dans un angle droit ?",
        hint: "90.",
        caption: "Tu valides ta réponse.",
      }),
    },
    {
      kind: "choice",
      kicker: "Étape 4 sur 6 · Unité",
      progress: 4,
      expected: "mètre",
      distractors: ["gramme", "litre"],
      copy: {
        football: {
          title: "Longueur",
          statement: "Quelle unité pour une longueur de terrain ?",
          hint: "mètre.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "Longueur",
          statement: "Quelle unité pour une longueur de terrain ?",
          hint: "mètre.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "Longueur",
          statement: "Quelle unité pour une longueur de terrain ?",
          hint: "mètre.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "Longueur",
          statement: "Quelle unité pour une longueur de terrain ?",
          hint: "mètre.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "number",
      kicker: "Étape 5 sur 6 · Périmètre",
      progress: 5,
      expected: "20",
      distractors: ["16", "24"],
      copy: allUniverses({
        title: "Carré côté 5",
        statement: "Périmètre d’un carré de côté 5 ?",
        hint: "4×5=20.",
        caption: "Tu valides ta réponse.",
      }),
    },
    {
      kind: "number",
      kicker: "Étape 6 sur 6 · Conversion",
      progress: 6,
      expected: "1000",
      distractors: ["100", "10"],
      copy: {
        football: {
          title: "m en mm…",
          statement: "1 m = combien de mm ?",
          hint: "1000.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "m en mm…",
          statement: "1 m = combien de mm ?",
          hint: "1000.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "m en mm…",
          statement: "1 m = combien de mm ?",
          hint: "1000.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "m en mm…",
          statement: "1 m = combien de mm ?",
          hint: "1000.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "number",
      kicker: "Application",
      progress: 6,
      expected: "45",
      distractors: ["40", "50"],
      copy: {
        football: {
          title: "3/4 d’heure",
          statement: "Les 3/4 d’une heure en minutes ?",
          hint: "45.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "3/4 d’heure",
          statement: "Les 3/4 d’une heure en minutes ?",
          hint: "45.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "3/4 d’heure",
          statement: "Les 3/4 d’une heure en minutes ?",
          hint: "45.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "3/4 d’heure",
          statement: "Les 3/4 d’une heure en minutes ?",
          hint: "45.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "continue",
      kicker: "Mission réussie",
      progress: 6,
      copy: {
        football: {
          title: "But ! Victoire !",
          statement: "Tes choix ont fait basculer le match. Les Bleus l’emportent.",
          caption: "Félicitations : tu as fait basculer le match.",
        },
        rugby: {
          title: "Essai ! Victoire !",
          statement: "Tes calculs ont ouvert l’intervalle. Essai transformé.",
          caption: "Félicitations : tu as fait basculer le match.",
        },
        equitation: {
          title: "Retour à l’écurie",
          statement: "Tu as lu chaque borne. Vous rentrez ensemble, en confiance.",
          caption: "Franchissement réussi, retour au pas jusqu’à l’écurie.",
        },
        espace: {
          title: "Arrimage réussi",
          statement: "La trajectoire est calée. Les attaches se verrouillent.",
          caption: "Le module est amarré. Mission réussie.",
        },
      },
    },
    {
      kind: "method",
      kicker: "Ce que tu as appris",
      progress: 6,
      copy: allUniverses({
        title: "Je progresse sur : Grandeurs (périmètre contenances durées)",
        statement: "Tu as entraîné la notion pas à pas, avec des exemples et des décisions.",
        note: "Relis les indices des étapes difficiles : ce sont de vraies méthodes à réutiliser.",
        caption: "Tu pourras réutiliser cette méthode dans une autre mission.",
      }),
    },
    {
      kind: "bilan",
      kicker: "Bilan sans note",
      progress: 6,
      copy: allUniverses({
        title: "Ton bilan",
        statement: "Tu as travaillé « Grandeurs (périmètre contenances durées) » sans note ni classement. Qu’as-tu réussi aujourd’hui ?",
        note: "Qu’as-tu préféré dans cette mission ?",
        caption: "Les réussites sont valorisées sans classement.",
      }),
    },
    {
      kind: "teaser",
      kicker: "Prochaine mission",
      progress: 6,
      copy: {
        football: {
          title: "Un nouveau défi t’attend",
          statement: "Au prochain match, un nouveau défi pédagogique t’attend.",
          caption: "Un point d’interrogation apparaît sur le terrain.",
        },
        rugby: {
          title: "Un nouveau défi t’attend",
          statement: "La prochaine rencontre apportera un nouveau défi.",
          caption: "Un nouveau schéma apparaît au loin.",
        },
        equitation: {
          title: "Un nouveau défi t’attend",
          statement: "Un nouveau chemin te conduira vers un autre défi.",
          caption: "Une rivière apparaît au loin.",
        },
        espace: {
          title: "Un nouveau défi t’attend",
          statement: "Un signal mystérieux annonce une prochaine mission.",
          caption: "Un point lumineux inconnu apparaît au-delà de la station.",
        },
      },
    },
  ],
});
