import { allUniverses, defineMission } from "../../define";

export const MISSION_ID = "ce2-questionner-le-monde-qdm-espace-temps-01";

/** CE2 / questionner-le-monde — Questionner le monde — Espace et temps. Phase A standard. */
export const mission = defineMission({
  id: MISSION_ID,
  grade: "ce2",
  subject: "questionner-le-monde",
  title: "Espace et temps en mission",
  blurb: "Une mission CE2 : Questionner le monde — Espace et temps.",
  available: false,
  version: 1,
  steps: [
    {
      kind: "continue",
      kicker: "Tutoriel",
      progress: 0,
      copy: allUniverses({
        title: "Bienvenue : Questionner le monde — Espace et temps",
        statement: "Dans cette mission de CE2, tu vas travailler : Questionner le monde — Espace et temps. Lis bien chaque consigne, utilise l’indice si besoin, puis valide.",
        hint: "Prends ton temps : il n’y a ni note ni classement.",
        caption: "La mission peut commencer.",
      }),
    },
    {
      kind: "continue",
      kicker: "Mise en action",
      progress: 0,
      copy: {
        football: {
          title: "Tu entres en jeu",
          statement: "Les Bleus comptent sur toi. Chaque bonne réponse fait avancer l’action sur le terrain.",
          caption: "Tu prends ta place au milieu du terrain.",
        },
        rugby: {
          title: "Tu entres en jeu",
          statement: "Tes partenaires comptent sur toi. Chaque bonne réponse ouvre un couloir.",
          caption: "Tu prends ta place derrière tes partenaires.",
        },
        equitation: {
          title: "Le chemin du retour",
          statement: "Tu rentres vers l’écurie avec ton cheval. Chaque étape éclaire le sentier.",
          caption: "L’écurie apparaît au bout du sentier.",
        },
        espace: {
          title: "Le retour vers la station",
          statement: "Le module doit rejoindre la station. Chaque réponse corrige la trajectoire.",
          caption: "La station apparaît au loin.",
        },
      },
    },
    {
      kind: "continue",
      kicker: "Objectif",
      progress: 0,
      copy: {
        football: {
          title: "Lis le jeu",
          statement: "Observe, calcule et décide. Chaque étape te rapproche du but. Objectif : Questionner le monde — Espace et temps.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "Observe la défense",
          statement: "Lis les espaces, calcule, puis avance au bon moment. Objectif : Questionner le monde — Espace et temps.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "Choisis le chemin",
          statement: "Observe les bornes, calcule, et garde une allure sûre. Objectif : Questionner le monde — Espace et temps.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "Rétablis la trajectoire",
          statement: "Analyse les signaux, calcule, et prépare l’arrimage. Objectif : Questionner le monde — Espace et temps.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "choice",
      kicker: "Étape 1 sur 6 · Temps",
      progress: 1,
      expected: "journée",
      distractors: ["fraction", "angle"],
      copy: allUniverses({
        title: "Repère",
        statement: "Matin, midi, soir font partie d’une…",
        hint: "journée.",
        caption: "Tu valides ta réponse.",
      }),
    },
    {
      kind: "choice",
      kicker: "Étape 2 sur 6 · Calendrier",
      progress: 2,
      expected: "mois",
      distractors: ["verbe", "solide"],
      copy: {
        football: {
          title: "Année",
          statement: "Une année est découpée en…",
          hint: "mois.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "Année",
          statement: "Une année est découpée en…",
          hint: "mois.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "Année",
          statement: "Une année est découpée en…",
          hint: "mois.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "Année",
          statement: "Une année est découpée en…",
          hint: "mois.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "choice",
      kicker: "Étape 3 sur 6 · Espace",
      progress: 3,
      expected: "carte",
      distractors: ["poème", "addition"],
      copy: allUniverses({
        title: "Se situer",
        statement: "Pour se situer, on peut utiliser une…",
        hint: "carte.",
        caption: "Tu valides ta réponse.",
      }),
    },
    {
      kind: "choice",
      kicker: "Étape 4 sur 6 · Orient",
      progress: 4,
      expected: "nord",
      distractors: ["verbe", "litre"],
      copy: {
        football: {
          title: "Point cardinal",
          statement: "Un point cardinal ?",
          hint: "nord.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "Point cardinal",
          statement: "Un point cardinal ?",
          hint: "nord.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "Point cardinal",
          statement: "Un point cardinal ?",
          hint: "nord.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "Point cardinal",
          statement: "Un point cardinal ?",
          hint: "nord.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "choice",
      kicker: "Étape 5 sur 6 · Hier",
      progress: 5,
      expected: "passé",
      distractors: ["futur", "présent seulement"],
      copy: allUniverses({
        title: "Temporalité",
        statement: "Hier appartient au…",
        hint: "passé.",
        caption: "Tu valides ta réponse.",
      }),
    },
    {
      kind: "choice",
      kicker: "Étape 6 sur 6 · Demain",
      progress: 6,
      expected: "futur",
      distractors: ["passé", "présent"],
      copy: {
        football: {
          title: "Temporalité",
          statement: "Demain appartient au…",
          hint: "futur.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "Temporalité",
          statement: "Demain appartient au…",
          hint: "futur.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "Temporalité",
          statement: "Demain appartient au…",
          hint: "futur.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "Temporalité",
          statement: "Demain appartient au…",
          hint: "futur.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "choice",
      kicker: "Application",
      progress: 6,
      expected: "semaine",
      distractors: ["siècle", "millénaire"],
      copy: {
        football: {
          title: "7 jours",
          statement: "Sept jours forment une…",
          hint: "semaine.",
          caption: "Les zones du terrain s’allument.",
        },
        rugby: {
          title: "7 jours",
          statement: "Sept jours forment une…",
          hint: "semaine.",
          caption: "Les couloirs gauche, axe et large deviennent visibles.",
        },
        equitation: {
          title: "7 jours",
          statement: "Sept jours forment une…",
          hint: "semaine.",
          caption: "Les principaux sentiers s’activent.",
        },
        espace: {
          title: "7 jours",
          statement: "Sept jours forment une…",
          hint: "semaine.",
          caption: "Chaque décision fait progresser le module.",
        },
      },
    },
    {
      kind: "continue",
      kicker: "Mission réussie",
      progress: 6,
      copy: {
        football: {
          title: "But ! Victoire !",
          statement: "Tes choix ont fait basculer le match. Les Bleus l’emportent.",
          caption: "Félicitations : tu as fait basculer le match.",
        },
        rugby: {
          title: "Essai ! Victoire !",
          statement: "Tes calculs ont ouvert l’intervalle. Essai transformé.",
          caption: "Félicitations : tu as fait basculer le match.",
        },
        equitation: {
          title: "Retour à l’écurie",
          statement: "Tu as lu chaque borne. Vous rentrez ensemble, en confiance.",
          caption: "Franchissement réussi, retour au pas jusqu’à l’écurie.",
        },
        espace: {
          title: "Arrimage réussi",
          statement: "La trajectoire est calée. Les attaches se verrouillent.",
          caption: "Le module est amarré. Mission réussie.",
        },
      },
    },
    {
      kind: "method",
      kicker: "Ce que tu as appris",
      progress: 6,
      copy: allUniverses({
        title: "Je progresse sur : Questionner le monde — Espace et temps",
        statement: "Tu as entraîné la notion pas à pas, avec des exemples et des décisions.",
        note: "Relis les indices des étapes difficiles : ce sont de vraies méthodes à réutiliser.",
        caption: "Tu pourras réutiliser cette méthode dans une autre mission.",
      }),
    },
    {
      kind: "bilan",
      kicker: "Bilan sans note",
      progress: 6,
      copy: allUniverses({
        title: "Ton bilan",
        statement: "Tu as travaillé « Questionner le monde — Espace et temps » sans note ni classement. Qu’as-tu réussi aujourd’hui ?",
        note: "Qu’as-tu préféré dans cette mission ?",
        caption: "Les réussites sont valorisées sans classement.",
      }),
    },
    {
      kind: "teaser",
      kicker: "Prochaine mission",
      progress: 6,
      copy: {
        football: {
          title: "Un nouveau défi t’attend",
          statement: "Au prochain match, un nouveau défi pédagogique t’attend.",
          caption: "Un point d’interrogation apparaît sur le terrain.",
        },
        rugby: {
          title: "Un nouveau défi t’attend",
          statement: "La prochaine rencontre apportera un nouveau défi.",
          caption: "Un nouveau schéma apparaît au loin.",
        },
        equitation: {
          title: "Un nouveau défi t’attend",
          statement: "Un nouveau chemin te conduira vers un autre défi.",
          caption: "Une rivière apparaît au loin.",
        },
        espace: {
          title: "Un nouveau défi t’attend",
          statement: "Un signal mystérieux annonce une prochaine mission.",
          caption: "Un point lumineux inconnu apparaît au-delà de la station.",
        },
      },
    },
  ],
});
